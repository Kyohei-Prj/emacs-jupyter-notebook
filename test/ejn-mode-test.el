...
(ert-deftest ejn-mode-test/undo-keybound-to-C-slash ()
  "C-/ should be bound to ejn-undo-command."
  (ejn-test-with-temp-buffer " *test*"
    (ejn-mode)
    (should (eq (lookup-key ejn-mode-map (kbd "C-/")) #'ejn-undo-command))
    (should (eq (lookup-key ejn-mode-map (kbd "C-x _")) #'ejn-undo-command))))
